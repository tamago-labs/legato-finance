...
module legato::amm_interface {
    ...
    public entry fun add_liquidity<X, Y>(
        ...
        let is_order = amm::is_order<X, Y>();

        if (!amm::has_registered<X, Y>(global)) {
            amm::register_pool<X, Y>(global, is_order)
        };
        let pool = amm::get_mut_pool<X, Y>(global, is_order);

        let (lp, return_values) = amm::add_liquidity(
            pool,
            coin_x,
            coin_x_min,
            coin_y,
        ...
        let global = amm::global_id<X, Y>(pool);
        let lp_name = amm::generate_lp_name<X, Y>();
    ...
    public entry fun remove_liquidity<X, Y>(
        ...
        let (coin_x, coin_y) = amm::remove_liquidity(pool, lp_coin, is_order, ctx);
    ...
    public entry fun swap<X, Y>(
        ...
        let return_values = amm::swap_out<X, Y>(
            global,
            coin_in,
            coin_out_min,
            is_order,
